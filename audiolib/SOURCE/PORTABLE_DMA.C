...
int DMA_SetupTransfer(int c,char *p,int len,int mode){ const dma_port_t *d; uint32_t a; unsigned sel,page,lo,hi,n;
 ...
 outp(d->address,lo);outp(d->address,hi);outp(d->page,page);outp(d->length,n&255u);outp(d->length,(n>>8)&255u);outp(d->mask,sel); return DMA_Ok; }
int DMA_EndTransfer(int c){const dma_port_t*d;unsigned sel;if(DMA_VerifyChannel(c)!=DMA_Ok)return DMA_Error;d=&ports[c];sel=(unsigned)c&3u;outp(d->mask,4u|sel);outp(d->clear,0);return DMA_Ok;}
char *DMA_GetCurrentPos(int c){const dma_port_t*d;uint32_t a;if(DMA_VerifyChannel(c)!=DMA_Ok)return 0;d=&ports[c];outp(d->clear,0);if(d->width){a=(uint32_t)inp(d->address)<<1;a|=(uint32_t)inp(d->address)<<9;}else{a=inp(d->address);a|=(uint32_t)inp(d->address)<<8;}a|=(uint32_t)inp(d->page)<<16;return (char*)dos_guest_linear_ptr(a);}
