...
static int service(void){if(playing&&callback)callback();return 0;}
int BLASTER_WriteDSP(unsigned v){unsigned n=65535;while(n--){if(!(inp(BLASTER_Config.Address+BLASTER_WritePort)&0x80)){outp(BLASTER_Config.Address+BLASTER_WritePort,v);return BLASTER_Ok;}TSM_Yield();}BLASTER_ErrorCode=BLASTER_CardNotReady;return BLASTER_Error;}
int BLASTER_ReadDSP(void){unsigned n=65535;while(n--){if(inp(BLASTER_Config.Address+BLASTER_DataAvailablePort)&0x80)return inp(BLASTER_Config.Address+BLASTER_ReadPort);TSM_Yield();}BLASTER_ErrorCode=BLASTER_CardNotReady;return BLASTER_Error;}
int BLASTER_ResetDSP(void){unsigned i;outp(BLASTER_Config.Address+BLASTER_ResetPort,1);for(i=0;i<8;i++)TSM_Yield();outp(BLASTER_Config.Address+BLASTER_ResetPort,0);for(i=0;i<100;i++){if(BLASTER_ReadDSP()==BLASTER_Ready)return BLASTER_Ok;TSM_Yield();}BLASTER_ErrorCode=BLASTER_CardNotReady;return BLASTER_Error;}
...
int BLASTER_GetCurrentPos(void){return playing?0:BLASTER_Error;}
static void start_dsp(int block){int count;if(version>=DSP_Version4xx){int cmd=(mix_mode&SIXTEEN_BIT)?DSP_16BitDAC:DSP_8BitDAC;int md=(mix_mode&STEREO?DSP_StereoBit:0)|(mix_mode&SIXTEEN_BIT?DSP_SignedBit:0);count=(mix_mode&SIXTEEN_BIT)?block/2-1:block-1;BLASTER_WriteDSP(cmd);BLASTER_WriteDSP(md);BLASTER_WriteDSP(count&255);BLASTER_WriteDSP((count>>8)&255);}else{count=block-1;BLASTER_WriteDSP(DSP_SetBlockLength);BLASTER_WriteDSP(count&255);BLASTER_WriteDSP((count>>8)&255);BLASTER_WriteDSP(DSP_8BitAutoInitMode);} }
int BLASTER_BeginBufferedPlayback(char*p,int n,int div,unsigned rate,int mode,void(*cb)(void)){int block,hz;BLASTER_StopPlayback();BLASTER_SetMixMode(mode);if(BLASTER_SetupDMABuffer(p,n,DMA_AutoInitRead)!=BLASTER_Ok)return BLASTER_Error;BLASTER_SetPlaybackRate(rate);callback=cb;block=n/div;if(block<=0)return BLASTER_Error;BLASTER_SpeakerOn();start_dsp(block);playing=1;hz=(int)(((uint64_t)sample_rate*(unsigned)packet_size()+block/2)/(unsigned)block);if(hz<1)hz=1;service_id=TSM_NewService(service,hz,1,0);if(service_id<0){BLASTER_StopPlayback();return BLASTER_Error;}return BLASTER_Ok;}
...
void BLASTER_StopPlayback(void){if(service_id>=0){TSM_DelService(service_id);service_id=-1;}if(playing){BLASTER_WriteDSP((mix_mode&SIXTEEN_BIT)?DSP_Halt16bitTransfer:DSP_Halt8bitTransfer);if(BLASTER_DMAChannel>=0)DMA_EndTransfer(BLASTER_DMAChannel);}playing=0;callback=0;}
